...
public class MongoGradeDB implements GradeDB {
    ...
    public Team getMyTeam() {
        ...
        try {
            ...
            if (responseBody.getInt("status_code") == 200) {
                JSONObject teamdata = responseBody.getJSONObject("team");

                String teamname = teamdata.getString("name");
                JSONArray memberlist = teamdata.getJSONArray("members");
                String[] members = new String[memberlist.length()];
                for (int i = 0; i < memberlist.length(); i++) {
                    members[i] = memberlist.getString(i);
                }

                Team.TeamBuilder teambuilder = Team.builder();
                teambuilder.name(teamname);
                teambuilder.members(members);
